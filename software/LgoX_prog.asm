...
; Timer2 management			; Timer 2 is used for led blink and keyboard read
...
;T2PR_INI    equ 0xFF	; T2PR 249 = 2000Hz; 166Hz per led
;;T2TMR = 0x00; TMR2 0; 
;T2CON_INI   equ b'00011000'	; T2CKPS 1:1; T2OUTPS 1:4; TMR2ON off; 
;T2CON8_INI   equ b'01111000'	; T2CKPS 1:1; T2OUTPS 1:16; TMR2ON off; 
;TMR_COUNT_INI	equ 0x05
...
NEWADR		; new address to be programmed 
...
PREAMB_LEN	;
PULSE_LEN	;
INT_COUNT	; counter in interrupt
...
ClearRam
	...
	comf SWITCHPORT,w	; read configuration switches
	andlw 0x0F			; mask switches only
	movwf NEWADR		; save as new address

MainLoop:
	movf DCCSENDL,w		; check for empty buffer
	btfss STATUS,Z
	goto MainLoop		; not empty --> do nothing

; 0x1A - this command enforce connected slave(s) to use address A. Command should be followed by one data byte 0x0A, where both A should be same.
;        slave present received address by blinking (number of blinks is current address)
;        in case A = 0, it mean no change of address, but blink current address (read address only)
	movlw 0x10			; set address command
	movwf DCCSEND3		; this command have always same XOR :)
	iorwf NEWADR,w		; add address
	movwf DCCSEND1		; command is 0x1A
	movf NEWADR,w		; data byte is 0x0A
	movwf DCCSEND2
	movlw 3				; 3 byte command
	movwf DCCSENDL		; 

	goto MainLoop		; and that is all...
